...
/*
1 - I did not get this message rotating along the X axis, but did once rotating
along the Y axis. My entire window went completely black before the message
appeared.
Upon making the fix, I continued to get this error -- however, I noticed that
the rotation happened much faster than it did on the first run of the code.
Edit: I had only fixed one of the two render calls. After fixing them, there
was no error. The rotation would pause once every turn or so, but would never
crash.
2, 3 - done, commented what I changed; I ended up having an error which the
developer tools said was part of a function I had not changed, in the flatten
function in MVnews.js and init.
*/
...
var numPlaneVertices  = 6; //numCubeVertices
...
// update cube rotation matrix (its instance transformation) then render ccube
    ...
    gl.drawArrays(gl.TRIANGLES, 0, numPlaneVertices); //numCubeVertices
...
// don't rotate traingle and render it
    ...
    gl.drawArrays(gl.TRIANGLES, numPlaneVertices, numTriangleVertices); //numCubeVertices
